...
import com.kospot.application.multi.game.StartRoadViewSoloGameUseCase;
...
@SpringBootTest
@ActiveProfiles("test")
public class StartRoadViewSoloGameUseCaseTest {
    ...
    void startGameSuccess() {
        ...
        // when
        MultiRoadViewGameResponse.StartPlayerGame response = startRoadViewSoloGameUseCase.execute(hostMember, request);
    ...
    void startGameNotHost() {
        ...
        assertThrows(Exception.class, () -> {
            startRoadViewSoloGameUseCase.execute(notHost, request);
        });
    }
    ...
    void startGameWithNonExistentRoom() {
        ...
        // when & then
        assertThrows(Exception.class, () -> {
            startRoadViewSoloGameUseCase.execute(hostMember, request);
        });
    }
    ...
    void startGameWithInsufficientPlayers() {
        ...
        // when & then
        assertThrows(Exception.class, () -> {
            startRoadViewSoloGameUseCase.execute(hostMember1, request);
        });
    }
        ...
        for (int i = 0; i < roomCount; i++) {
            ...
            executorService.execute(() -> {
                try {
                    ...
                    startRoadViewSoloGameUseCase.execute(hosts.get(index), request);
                    successCount.incrementAndGet();
                } catch (Exception e) {

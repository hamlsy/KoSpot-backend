...
@SpringBootTest
@ActiveProfiles("test")
public class StartRoadViewSoloRoundUseCaseTest {
    ...
    void startGameSuccess() {
        ...
        // when
        MultiRoadViewGameResponse.StartPlayerGame response = startRoadViewSoloRoundUseCase.execute(hostMember, request);
    ...
    void startGameNotHost() {
        ...
        assertThrows(Exception.class, () -> {
            startRoadViewSoloRoundUseCase.execute(notHost, request);
        });
    }
    ...
    void startGameWithNonExistentRoom() {
        ...
        // when & then
        assertThrows(Exception.class, () -> {
            startRoadViewSoloRoundUseCase.execute(hostMember, request);
        });
    }
    ...
    void startGameWithInsufficientPlayers() {
        ...
        // when & then
        assertThrows(Exception.class, () -> {
            startRoadViewSoloRoundUseCase.execute(hostMember1, request);
        });
    }
        ...
        for (int i = 0; i < roomCount; i++) {
            ...
            executorService.execute(() -> {
                try {
                    ...
                    startRoadViewSoloRoundUseCase.execute(hosts.get(index), request);
                    successCount.incrementAndGet();
                } catch (Exception e) {

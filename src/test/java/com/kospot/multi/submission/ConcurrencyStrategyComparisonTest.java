...
class ConcurrencyStrategyComparisonTest {
    ...
    void printReport() {
        ...
        if (mostAccurate != null) {
            ...
            log.info("\n🎯 가장 정확한 전략: {} (성공률 {:.1f}%)",
                    accurateName, mostAccurate.getSuccessRate());
        }

        if (fastest != null) {
            String fastestName = findStrategyName(fastest);
            log.info("⚡ 가장 빠른 전략: {} (평균 {:.2f}ms)",
                    fastestName, fastest.getAvgDuration());
        }
    ...
    private StrategyResult runStrategy(String name, Supplier<ConcurrencyTestResult> strategy) {
        ...
        log.info("-".repeat(80));
        log.info("   결과: 성공률 {:.1f}%, 평균 {:.2f}ms, 최소 {:.2f}ms, 최대 {:.2f}ms",
                result.getSuccessRate(), avgDuration, minDuration, maxDuration);
    ...
    private ConcurrencyTestResult simulateConcurrentAccess(Supplier<Boolean> action) {
        ...
        try {
            ...
            log.error("Timeout waiting for threads", e);
        }
